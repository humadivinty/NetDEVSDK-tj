...
Camera6467_plate::~Camera6467_plate()
{
    ...
    safeCloseThread(m_hStatusCheckThread);
    SetConnectStatus_Callback(NULL, NULL, 10);    
    //StopPlayVideo();
    ...
    SAFEDELETE_OBJ(m_CameraResult);
    SAFEDELETE_OBJ(m_BufferResult);

    SAFEDELETE_ARRAY_OBJ(m_pTempBin);
    SAFEDELETE_ARRAY_OBJ(m_pTempBig1);
    SAFEDELETE_ARRAY_OBJ(m_pVideoImgBuffer);
...
int Camera6467_plate::StartPlayVideo(int iChannelID, HANDLE& playHandle,const HWND winHandle)
{
    ...
    if (iChannelID == 0)
    {
        sprintf_s(chCMD, "rtsp://%s:554/h264ESVideoTest", m_strIP.c_str());
        //m_iVedioChannelID = 0;
    }
    else
    {
        sprintf_s(chCMD, "rtsp://%s:554/h264ESVideoTestSecond", m_strIP.c_str());
        //m_iVedioChannelID = 1;
    }
    WriteLog(chCMD);
